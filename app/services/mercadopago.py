...
def mercadopago_is_test_environment():
    return mercadopago_environment() == "test"


def mercadopago_token_fingerprint():
    token = mercadopago_access_token() or ""
    if not token:
        return "sem-token"
    prefix = token.split("-", 1)[0] if "-" in token else token[:6]
    return f"{prefix}...{token[-6:]}"


def is_live_credentials_error(error_payload):
    ...
    return "unauthorized use of live credentials" in f"{message} {cause_text}".lower()


def mercadopago_test_refund_can_be_local_only(error_payload):
    # Em teste/sandbox nao ha dinheiro real; se o MP recusar o estorno por
    # ambiente/credencial, ainda liberamos o teste do fluxo administrativo.
    return mercadopago_is_test_environment() and is_live_credentials_error(error_payload)
...
def humanize_mercadopago_error(error_payload):
    ...
    if "unauthorized use of live credentials" in combined:
        return (
            ...
            f"Ambiente atual: {mercadopago_environment()} | token carregado: {mercadopago_token_fingerprint()}. "
            "Use o ACCESS TOKEN da mesma aplicação e do mesmo ambiente em que o pagamento foi criado. "
            "Em ambiente de teste, o sistema pode reabrir a fatura localmente porque não há dinheiro real."
        )
